...
const Profile = () => {
  ...
  useEffect(() => {
    async function getProfile() {
	 ...
	 setCharityEmail(res.email)
	 setCharityPhone(res.phone)
	 setCharityAbout(res.about)
    }
	getProfile();
  }, []);
  ...
  const { config } = usePrepareContractWrite({
    address: contractAddress,
    abi: charityAbi,
    functionName: 'charityUpdateProfile',
  ...
  if(useContractWriteData?.hash && !isLoading){
		toast({
			title: 'Charity profile updated successfully.',
			description: "",
			status: 'success',
			position: 'bottom',
			duration: 2000,
  ...
  return (
	...
	<div className='tabbed-bar'>
		<ul>
		  <li><Link href='/account/profile' className='active'>Account</Link></li>
